...
if [ ! -f "${HOME}/.spack/config.yaml" ]; then
    mkdir -p "${HOME}/.spack"
    cp /opt/nesi/spack/pure/etc/spack-conf/user/config.yaml "${HOME}/.spack/config.yaml"
    cp /opt/nesi/spack/pure/etc/spack-conf/user/upstreams.yaml "${HOME}/.spack/upstreams.yaml"
    cp /opt/nesi/spack/pure/etc/spack-conf/user/modules.yaml "${HOME}/.spack/modules.yaml"
    echo ""
